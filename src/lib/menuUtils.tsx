...
export async function fetchMenuData() {
  ...
  // 3. Fetch categorías
  const { data: enumCategory, error: enumError } = await supabase.rpc('get_enumcategories_values');
  ...
  dishes = dishes.filter((dish) => !promoDishIds.includes(dish.id));

  // 5. Agrupar platos por categoría
  const dishesByCategory = Object.fromEntries(
    (enumCategory as string[]).map((cat) => [cat, dishes.filter((dish) => dish.category === cat)])
  );
...
export async function getCategorizedDishes() {
  try {
    ...
    const { data: enumCategory, error: enumError } = await supabase.rpc('get_available_categories');
    ...
    console.error('Error getting categorized dishes:', errorMessage);
    return { categorizedDishes: {}, error: errorMessage };
  }
}
